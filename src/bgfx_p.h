...
#define BGFX_UNIFORM_GEOMETRYBIT UINT8_C(0x40)
#define BGFX_UNIFORM_MASK (BGFX_UNIFORM_FRAGMENTBIT|BGFX_UNIFORM_SAMPLERBIT|BGFX_UNIFORM_GEOMETRYBIT)
...
//			}
	...
	struct BX_NO_VTABLE RendererContextI
	{
		...
		virtual void* createTexture(TextureHandle _handle, const Memory* _mem, uint64_t _flags, uint8_t _skip, bool _genMips) = 0;
...
#	define BGFX_API_FUNC(_func) _func
	...
	struct Context
	{
		...
		BGFX_API_FUNC(TextureHandle createTexture(const Memory* _mem, uint64_t _flags, uint8_t _skip, bool _genMips, TextureInfo* _info, BackbufferRatio::Enum _ratio, bool _immutable) )
		{
			BGFX_MUTEX_SCOPE(m_resourceApiLock);
